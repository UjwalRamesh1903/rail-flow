...
function deterministicSeats(seed: number) {
  return 8 + ((seed * 17) % 47)
}

function makeClasses(multiplier: number, seed = 1) {
  return classTemplates.map((c, i) => {
    const availability = availabilities[(i + seed) % availabilities.length] as (typeof availabilities)[number]
    return {
      code: c.code,
      name: c.name,
      fare: Math.round(c.baseFare * multiplier),
      availability,
      seats: availability === 'Available' ? deterministicSeats(seed + i) : undefined,
    }
  })
}
...
export const trains: Train[] = [
  ...
  },
  {
    ...
    runningDays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    classes: makeClasses(0.9, 1).filter((c) => ['CC', 'EC', '2S'].includes(c.code)),
  },
  {
  ...
  },
  {
    ...
    classes: makeClasses(0.7, 1).filter((c) => ['3A', 'CC'].includes(c.code)),
  },
  {
...
export function getTrainsForRoute(fromCode: string, toCode: string): Train[] {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    routeTrains.push({
      ...
      number: String(12000 + i * 111 + ((fromCode.charCodeAt(0) + toCode.charCodeAt(0) + i) % 50)),
      ...
      classes: makeClasses(0.8 + (i % 5) * 0.1, i + fromCode.length + toCode.length),
    })
  }
