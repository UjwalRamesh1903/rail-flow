...
export function TrainsPage() {
  ...
  if (!search.from || !search.to) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        <p className="text-gray-400 mb-4">Please search for trains from the homepage first.</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      {/* Search summary */}
      <div className="bg-[#1a2332] rounded-2xl border border-white/10 p-4 mb-6">
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <span className="font-bold text-gray-100">{search.from.name}</span>
          <span className="px-1.5 py-0.5 bg-irctc-blue-light text-irctc-blue text-xs font-bold rounded">{search.from.code}</span>
          <ArrowRight className="w-4 h-4 text-gray-400" />
          <span className="font-bold text-gray-100">{search.to.name}</span>
          <span className="px-1.5 py-0.5 bg-irctc-blue-light text-irctc-blue text-xs font-bold rounded">{search.to.code}</span>
          <span className="text-gray-400">|</span>
          <span className="text-gray-400">{search.date ? formatJourneyDate(search.date) : ''}</span>
          <span className="text-gray-400">|</span>
          <span className="text-gray-400">{search.adults + search.children} Passenger(s)</span>
      ...
      <div className="flex gap-6">
        ...
        <aside className="hidden lg:block w-64 shrink-0">
          <div className="bg-[#1a2332] rounded-2xl border border-white/10 p-4 sticky top-20">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm text-gray-400">{filteredTrains.length} train(s) found</p>
            <button
          ...
          {showFilters && (
            <div className="lg:hidden bg-[#1a2332] rounded-2xl border p-4 mb-4 animate-slide-down">
          ...
          {filteredTrains.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredTrains.map((train) => (
                <div key={train.id} className="bg-[#1a2332] rounded-2xl border border-white/10 p-4 lg:p-5 hover:shadow-md transition-shadow">
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-gray-100">{train.number}</span>
                        <span className="text-gray-400 text-sm">{train.name}</span>
                        <span className="px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-medium rounded">{train.type}</span>
                      </div>
                      <div className="flex items-center gap-4 mt-2">
                        <div>
                          <div className="text-lg font-bold text-gray-100">{train.departure}</div>
                          <div className="text-xs text-gray-400">{train.fromCode}</div>
                        </div>
                        <div className="flex-1 flex flex-col items-center">
                          <div className="text-xs text-gray-400 flex items-center gap-1">
                            <Clock className="w-3 h-3" /> {train.duration}
                          </div>
                          <div className="w-full h-px bg-white/10 my-1 relative">
                        ...
                        <div className="text-right">
                          <div className="text-lg font-bold text-gray-100">{train.arrival}</div>
                          <div className="text-xs text-gray-400">{train.toCode}</div>
                        </div>
                      </div>
                    </div>
                        ...
                        .map((cls) => (
                          <button
                            ...
                            className={cn(
                              ...
                              cls.availability === 'Not Available' && 'border-white/10 bg-[#0a0e17] opacity-50 cursor-not-allowed',
                            )}
                          >
                            <div className="font-bold text-gray-200">{cls.code}</div>
                            <div className={cn(
                              'font-medium',
