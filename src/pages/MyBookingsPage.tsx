      ...
      {uniqueBookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {uniqueBookings.map((booking) => (
            <div key={booking.pnr} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-gray-900">{booking.trainNumber}</span>
                    <span className="text-sm text-gray-600">{booking.trainName}</span>
